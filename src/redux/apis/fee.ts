...
export const createFee = async (year: number, semester: number, amount: number) => {
  return request.post('/fee/create', {year, semester, amount});
};

// 2. Amount - 회비 납부액 조회
export const amountListSearch = async (id: string, year: number, semester: number) => {
  return request.post('/fee/amount', {id, year, semester});
};

// 3. Payers - 회비 납부자 목록 조회
export const payersListSearch = async (year: number, semester: number) => {
...
export const searchTransaction = async (year: number, semester: number) => {
