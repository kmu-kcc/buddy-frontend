...
import {getWithdrawalRequests} from '../../store/actions/memberActions';
...
export const WithdrawRequests = () => {
  ...
  const {withdrawalRequests} = useSelector((state: RootState) => state.member);
  ...
  const CardList = withdrawalRequests.map((u) => {
    return {
      ...u,
      checked: false,
    };
